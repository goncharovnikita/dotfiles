local function new()
	...
	function buf_util.create_buffer_handler(buf)
		...
		function bh.with_debug_info(cb) debug_info_cb = cb end

		function bh.wipe() wipe_curr_lines() end
		...
		function bh.write(text)
			...
			if debug_info_cb then text = debug_info_cb() .. text end
		...
		function bh.append(text)
			...
			if curr_lines_count == 0 and debug_info_cb then text = debug_info_cb() .. text end
		...
		function bh.attach_to_win(win) vim.api.nvim_win_set_buf(win, buf) end

		function bh.set_filetype(ft) vim.api.nvim_buf_set_option(buf, "filetype", ft) end

		function bh.set_name(name) vim.api.nvim_buf_set_name(buf, name) end

		function bh.get_buffer() return buf end
